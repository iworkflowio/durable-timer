...
type TimerQueue interface {
	...
	// 4. Have a background thread to scan the list to find out the timers that are completed, and pass the info to TimerBatchDeleter to delete the timers from database
}
...
type TimerBatchReader interface {
	...
	// 3. Listen to TimerQueue to know when to read next batch of timers (with some delay to control the rate of reading)
