...
func TestClaimShardOwnership_NewShard(t *testing.T) {
	...
	query := "SELECT shard_version, shard_owner_id, shard_metadata, shard_claimed_at FROM timers WHERE shard_id = $1 AND row_type = $2 AND timer_execute_at = $3 AND timer_uuid = $4"
...
func TestClaimShardOwnership_ExistingShard(t *testing.T) {
	...
	var dbVersion int64
	var dbOwnerId string
	query := "SELECT shard_version, shard_owner_id FROM timers WHERE shard_id = $1 AND row_type = $2 AND timer_execute_at = $3 AND timer_uuid = $4"
...
func TestClaimShardOwnership_NilMetadata(t *testing.T) {
	...
	var dbMetadata *string
	query := "SELECT shard_metadata FROM timers WHERE shard_id = $1 AND row_type = $2 AND timer_execute_at = $3 AND timer_uuid = $4"
