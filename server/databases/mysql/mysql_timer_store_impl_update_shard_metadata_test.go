...
func TestUpdateShardMetadata_IdenticalValues(t *testing.T) {
	...
	// Now update with EXACTLY the same metadata values (should succeed with clientFoundRows=true)
	...
	assert.Nil(t, updateErr2, "Update with identical values should succeed when clientFoundRows=true")
