...
func TestRangeDeleteWithLimit_Basic(t *testing.T) {
	...
	// Delete with limit 3 - should delete first 3 timers by ExecuteAt order (MySQL now respects limit)
	...
	// Verify first 3 timers are deleted, last 2 remain (MySQL now respects limit)
	for i := 0; i < 5; i++ {
...
func TestRangeDeleteWithLimit_NoLimit(t *testing.T) {
	...
	// Delete with high limit (effectively no limit) - should delete all timers in range
	deleteRequest := &databases.RangeDeleteTimersRequest{
		StartTimestamp: now,
		StartTimeUuid:  databases.ZeroUUID,
		EndTimestamp:   now.Add(10 * time.Minute),
	...
	response, deleteErr := store.RangeDeleteWithLimit(ctx, shardId, deleteRequest, 1000)
	assert.Nil(t, deleteErr)
	require.NotNil(t, response)
