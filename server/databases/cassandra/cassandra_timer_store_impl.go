...
func (c *CassandraTimerStore) ClaimShardOwnership(
	...
	if errors.Is(err, gocql.ErrNotFound) {
		...
		if !applied {
			// Another instance created the record concurrently, give up to the other instance
			return 0, databases.NewDbErrorOnShardConditionFail("failed to insert shard record due to concurrent insert", nil)
		} else {
	...
	if !applied {
		// Version changed concurrently, operation failed
		return 0, databases.NewGenericDbError("shard ownership claim failed due to concurrent modification", nil)
	}
